...
- **Environments:** `.env` via `getEnv()` in `src/ui/helpers/env/`
...
|----------------|---------------|----------------|
| **Feature (Gherkin)** | `src/ui/tests/features/<module>.feature` | One Feature per module; scenarios use Given/When/Then; tag with `@<module>` or `@smoke` or `@regression`etc. |
| **Step definitions** | `src/ui/stepDefinitions/<module>.steps.ts` (or add to existing) | Bind to Gherkin steps; call Page Objects and assertions; use `this` (World) for shared context if needed. |
| **Page Object(s)** | `src/ui/pages/` | One or more POM classes; e.g. `LoginPage.ts`, `PimListPage.ts`. Encapsulate selectors and UI actions only. |
| **UI test data** | `src/ui/tests/data/<module>.ts` | Export constants/objects for inputs, labels, expected text (e.g. credentials, form data). |
| **API test data** | `src/ui/tests/data/apiData/<module>.ts` | Payloads and endpoint-related data for API tests. |
| **Types** | `src/ui/types/<module>.types.ts` | TypeScript interfaces/types for that module (form data, API response, etc.). |
| **Assertions** | `src/ui/utils/assertions/<module>Assertions.ts` | Custom assertion helpers (e.g. `assertLoginSuccess`, `assertPimRecordVisible`). Use Playwright `expect` inside. |
| **Selectors (if new)** | `src/ui/constants/selectors/` | Shared selector constants used by POMs (e.g. button IDs, data-testid). |
...
- **Step definitions**
  ...
  - Use `getEnv()` for base URL and config (from `src/ui/helpers/env/getEnv.ts`).
- **Page Objects**
  - Accept `page` (Playwright) in constructor; expose methods like `goto()`, `fillUsername()`, `clickSubmit()`.
  - Use locators from `src/ui/constants/selectors/` or `src/ui/utils/locator/wrapperLocator.ts` when applicable.
- **Test data**
  - No hardcoded credentials in code; use `src/ui/tests/data/<module>.ts` or env (e.g. `getEnv('USERNAME')`).
- **Assertions**
...
- Put **API payload/endpoint data** in `src/ui/tests/data/apiData/<module>.ts`.
- Use **API helpers** from `src/ui/utils/api/apiMethods.ts` (GET/POST/PUT) in step definitions.
- Optionally add **types** in `src/ui/types/<module>.types.ts` for request/response.
- Step definitions can live in the same `src/ui/stepDefinitions/<module>.steps.ts` or a dedicated `src/ui/stepDefinitions/api.<module>.steps.ts` as per your convention.

### Step 5 — If the scenario is UI + API (e.g. login via API then UI check)

- Combine: use API data from `src/ui/tests/data/apiData/`, API methods from `src/ui/utils/api/`, and UI steps using POMs and `src/ui/utils/assertions/`.
...
- Follow the paths and naming in TEST-GENERATOR-PROMPT.md (features in src/ui/tests/features/, steps in src/ui/stepDefinitions/, pages in src/ui/pages/, data in src/ui/tests/data/ and src/ui/tests/data/apiData/, types in src/ui/types/, assertions in src/ui/utils/assertions/).
...
- [ ] Feature file exists/updated in `src/ui/tests/features/<module>.feature` with clear scenarios.
- [ ] Step definitions in `src/ui/stepDefinitions/` implement every step used in the feature.
- [ ] Page Object(s) in `src/ui/pages/` with no assertions (only actions and locators).
- [ ] UI test data in `src/ui/tests/data/<module>.ts`; API data in `src/ui/tests/data/apiData/<module>.ts` if needed.
- [ ] Types in `src/ui/types/<module>.types.ts` where applicable.
- [ ] Custom assertions in `src/ui/utils/assertions/<module>Assertions.ts`; step defs use them instead of inline `expect`.
- [ ] No hardcoded URLs/credentials; use `getEnv()` or data files.
- [ ] Hooks/World in `src/ui/helpers/hooks/` are used if the scenario needs shared setup (e.g. login state).
