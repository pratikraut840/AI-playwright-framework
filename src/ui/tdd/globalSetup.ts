...
const AUTH_STATE_PATH = path.join('src', 'ui', 'helpers', 'setupLogin', 'auth', 'admin-user.json');

async function globalSetup(): Promise<void> {
  clearTddResultDirs();
  const ENV_NAME = process.env.NODE_ENV ?? 'dev';
  dotenv.config({ path: path.join('src', 'ui', 'helpers', 'env', `.env.${ENV_NAME}`) });
  dotenv.config({ path: path.join('src', 'ui', 'helpers', 'env', '.env') });
